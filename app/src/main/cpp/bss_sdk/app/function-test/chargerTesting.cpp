...
#define CHARGER_TEST_TIME   (2*60*1000)
#define CAB_TEST_NUMBER     2
#define CHARGER_TEST_ID     0
...
int main() {
    ...
    WaitTimer m_timeout{CHARGER_TEST_TIME};
    WaitTimer m_printTime{1000};
    int32_t cabIndex = 0;
    ...
    while (true){
        ...
        if(!m_timeout.getRemainTime()){
            chargerCharge(cabIndex);

            cabIndex++;
            if(cabIndex >= CAB_TEST_NUMBER){
                cabIndex = 0;
            }
        ...
        if(chargerGetState(CHARGER_TEST_ID) == CHARGER_STATE::CHARGER_STATE_CHARGED && !m_printTime.getRemainTime()){
            BpSyncData _data;
            bpSyncData(_data);
            test_printBpSyncData(_data);
