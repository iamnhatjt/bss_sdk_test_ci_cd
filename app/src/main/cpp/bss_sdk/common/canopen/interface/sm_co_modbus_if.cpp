...
sm_co_if_t *sm_co_if_create_default(uint8_t _type,  const char *_argv, int _argc, void* _arg){
    (void*)_argv;
    if(_type == CO_MODBUS_CANBUS_IF) {
        g_canopen_if.m_current_address = _argc;
        g_canopen_if.m_modbus_master = _arg;
        return &g_canopen_if;
    }
    return NULL;
}
...
int sm_co_if_set_config(sm_co_if_t *self, const char *_argv, int _argc, void* _arg){
    ...
    self->m_current_address = _argc;
    self->m_modbus_master = _arg;
